...
const AnalysisLoader: React.FC<AnalysisLoaderProps> = ({ title, steps }) => {
    const [currentStep, setCurrentStep] = useState(0);

    useEffect(() => {
        const stepDuration = Math.max(1500, 8000 / steps.length); // Make it feel substantial
    ...
    return (
        <div className="flex flex-col items-center justify-center h-full text-center p-4 bg-white rounded-lg min-h-[400px]">
            ...
            <p className="text-gray-500 mt-1">Please wait, the AI is processing your request...</p>
                        ...
                        <div className="w-5 h-5 mr-3 flex-shrink-0 flex items-center justify-center">
                            {index < currentStep ? (
                                ...
                                <div className="w-4 h-4 relative">
                                    <span className="w-full h-full bg-gray-300 rounded-full animate-pulse"></span>
                                </div>
                            )}
                        </div>
